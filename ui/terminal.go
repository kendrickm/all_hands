...
func getOutputTextRect(insetTerminal *sdl.Rect) *sdl.Rect {
	terWidth  := int32(float32(insetTerminal.W) * 0.18)
	terHeight := int32(float32(insetTerminal.H) * 0.18)
	offsetX := insetTerminal.X+int32(float32(insetTerminal.W)*0.5) - terWidth/2
	offsetY := insetTerminal.Y+insetTerminal.H-terHeight-int32(float32(insetTerminal.H) * 0.16)
	return &sdl.Rect{X:offsetX, Y: offsetY, W: terWidth, H: terHeight} 
}
